            ...
            <section className="flex flex-col space-y-2">
                {
                    images.map((image, index) => (
                        <div key={index}
                            className={`w-[40px] h-[55px] rounded-lg border hover:border-green-600 cursor-pointer ${selectimage === image ? 'border-blue-500' : ''}`}
                            onClick={() => handleselectimage(image)}
                        >
                            <Image src={image} alt="alt" layout='responsive' objectFit='cover' objectPosition='center' />
                        </div>
                    ))
                }
            </section>
            <section className='flex items-center gap-2'>
                <div className='relative w-12 h-12 rotate-180 cursor-pointer'
                    onClick={handleprevimage}
                >
                    <Image
                        src={rightarrow}
                        className='hover:bg-slate-400 rounded-full'
                        alt="alt" layout='fill' objectFit='cover' objectPosition='center'
                    />
                </div>
